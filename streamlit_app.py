...
if uploaded:
    ...
    # Convertir indicadores a porcentaje 0-100 aunque vengan mezclados
    def limpiar_porcentaje(col):
        s = col.astype(str).str.replace("%", "", regex=False).str.replace(",", ".", regex=False)
        s = pd.to_numeric(s, errors="coerce")
        return s.apply(lambda x: x * 100 if pd.notna(x) and abs(x) <= 1.5 else x)

    for c in [COL_DISP, COL_EFIC, COL_CAL, COL_OEE]:
        df[c] = limpiar_porcentaje(df[c])
